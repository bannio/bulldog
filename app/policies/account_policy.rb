class AccountPolicy < ApplicationPolicy
  ...
  def permitted_attributes
    [:name, :address, :postcode,
      ...
      :stripe_card_token, :title, :price, :interval, :card_last4, :state,
      :card_expiration, :next_invoice, :date_reminded, :mail_list, :trial_end]
  end
  ...
  def edit?        ; true                                  ; end
  def cancel?
    user.account.active? || user.account.paying?
  end
